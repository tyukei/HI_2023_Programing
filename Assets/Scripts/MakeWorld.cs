...
public class MakeWorld : MonoBehaviour
{
  ...
  int r; //乱数の値
  Object[] wall = new Object[3]; //壁オブジェクト
  ...
  void Start()
  {
    ...
    field[max_z-1,max_x-2] = 2; //ゴール地点に旗を置く準備をする。
    ...
    for(z=0; z<max_z; z=z+1) //フィールドの縦幅の分だけループする。
    {
      for(x=0; x<max_x; x=x+1) //フィールドの横幅の分だけループする。
      {
        ...
        {
          if(x == 0 || x == max_x-1){
            wallgo = (GameObject)Instantiate(wall[0], new Vector3(5.0f*x,0.0f,5.0f*z), Quaternion.identity); //壁を配置する。
          }else if(z == 0 || z == max_z-1){
            //wall[0]を90度回転した壁を配置する。
            wallgo = (GameObject)Instantiate(wall[0], new Vector3(5.0f*x,0.0f,5.0f*z), Quaternion.Euler(0.0f,90.0f,0.0f)); //壁を配置する。
          }else{
            wallgo = (GameObject)Instantiate(wall[1], new Vector3(5.0f*x,0.0f,5.0f*z), Quaternion.identity); //壁を配置する。
          }
        }
        else if(field[z,x]==2) // 旗なら
        {
